...
class AutoLineModel(models.Model):
    ...
    brand = models.ForeignKey(
        AutoBrand, on_delete=models.CASCADE, related_name="model_lines"
    )
...
class Auto(models.Model):
    ...
    line_model = models.ForeignKey(
        AutoLineModel, on_delete=models.CASCADE, related_name="models"
    )
    start_production = models.IntegerField(
        ("year"), choices=year_choices(), default=current_year()
    )
    end_production = models.IntegerField(
        ("year"), choices=year_choices(), default=current_year()
    )
...
class ReviewAuto(models.Model):
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name="AutoReviews", on_delete=models.CASCADE
    )
    title = models.CharField(max_length=128)
    review = models.TextField()
    plus = models.CharField(max_length=256)
    minus = models.CharField(max_length=256)
    rating_hod = models.PositiveIntegerField(
        choices=[(i, i) for i in range(1, 6)], verbose_name="Рейтинг"
    )
    rating_komf = models.PositiveIntegerField(
        choices=[(i, i) for i in range(1, 6)], verbose_name="Рейтинг"
    )
    rating_nade = models.PositiveIntegerField(
        choices=[(i, i) for i in range(1, 6)], verbose_name="Рейтинг"
    )
    rating_odsl = models.PositiveIntegerField(
        choices=[(i, i) for i in range(1, 6)], verbose_name="Рейтинг"
    )
    rating_cost = models.PositiveIntegerField(
        choices=[(i, i) for i in range(1, 6)], verbose_name="Рейтинг"
    )
