...
class ToBasketUserAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.user_profile = UserProfile.objects.create(
            user=self.user, is_client=True, money_in_wallet=10000
        )
        ...
        self.vendor_1 = Vendor.objects.create(
            name="Vendor 1", email="test@test.com", address="test st 32"
        )
    ...
    def test_add_to_bad_offer_basket(self):
        response = self.client.post(
            reverse("ProductToBasketViewSet-list"),
    ...
    def test_add_to_basket(self):
        self.offer_1 = OfferVendor.objects.create(
            product=self.product_1, vendor=self.vendor_1, price=100
        )
        response = self.client.post(
            reverse("ProductToBasketViewSet-list"),
            data=json.dumps(self.data),
    ...
    def test_add_more_to_basket(self):
        ...
        self.pib = ProductInBasket.objects.create(
            basket=self.basket_user, offer=self.offer_1, count=5
        )
    ...
    def test_delete_from_bad_pib_basket(self):
        response = self.client.delete(
            reverse("ProductToBasketViewSet-detail", args=[50])
        )
    ...
    def test_delete_from_basket(self):
        ...
        response = self.client.delete(
            reverse("ProductToBasketViewSet-detail", args=[self.pib.id])
        )
        self.pid_2 = ProductInBasket.objects.get(pk=self.pib.id)
    ...
    def test_delete_from_basket_zero_count(self):
        ...
        self.pib = ProductInBasket.objects.create(
            basket=self.basket_user, offer=self.offer_1, count=0
        )
        response = self.client.delete(
            reverse("ProductToBasketViewSet-detail", args=[self.pib.id])
        )
